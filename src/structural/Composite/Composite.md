...
- We have a cart (Amazon), it can consist of either a Product or a Product Bundle. Now if I am dealing with the cart, I have to explicitly chekc if it's a product or bundel before doing anything on it. Also the logic of adding any object to cart has to be generalised to list<Object> so it can accept any of these types, which is not a good practice. When performing operations on these objects, I ahve to explicitly cast them to their specific type and then do getPrice or similar ops.
